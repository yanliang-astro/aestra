...
def plot_diagnostic(diags,instrument,ratio=50,n_window=2):
    import matplotlib.pyplot as plt
    from scipy.ndimage import gaussian_filter1d
    from astropy.timeseries import LombScargle
    from util import moving_mean
    if "rv" in diags:
        time,v_encode,v_planet,v_act,v_doppler = [tensor2array(item[:,0]) for item in diags["rv"]]
    ...
    spec_zero_rv,w,spec_aug = [tensor2array(item.squeeze(1)) for item in diags["input_data"]]
    ...
    for k in range(ncols):
        center = wave_obs[wh]
        ax = axs[0]#[k]
        spec_inflate = ratio*spec_zero_rv[i]+spec_rest
        #aug_inflate = ratio*spec_aug[i]+template
        model_inflate = ratio*model_resid[i]+spec_rest
        #spec[w[i]<1] = 0
        #ax.plot(wave_obs,template,"-",color='navy',drawstyle=drawstyle,label="template",zorder=zorder)
        ax.plot(wave_obs,spec_rest,"-",color='b',drawstyle=drawstyle,label="spec_rest",zorder=zorder)
        ...
        err = ratio*(w[i]**(-0.5))
        ax.fill_between(wave_obs,spec_inflate-err,spec_inflate+err,
...
def print_planet_solutions(p0,p,n_string=10,corr=None):
    rank = torch.argsort(p[:len(p0),0],descending=True)
    for i in rank:
        K0,P0 = p0[i,:2]
        K,P = p[i,:2]
        str1 = f"{P0:.2f}d"
...
def get_losses(model,
    ...
    spec_input, w, _ = interpolate_to_input_grid(batch,instrument,template_data,extra_rv=v_planet)
    #  generate augment spectra
    spec_input_aug,w_aug,z_off_true = interpolate_to_input_grid(batch,instrument,template_data,aug=True,extra_rv=v_planet)
    ...
    print("rv_err:",rv_err.min().item(),rv_err.max().item())
    z = (rv)/instrument.c

    rv_aug,_ = model.estimate_rv(spec_input_aug)
    ...
    if stellar_activity:
        ...
        spec_zero_rv, w, _ = interpolate_to_input_grid(batch,instrument,template_data,extra_rv=(v_planet-rv))
        ...
        fid_loss = 4*model._loss(spec_zero_rv, w, model_resid)
    ...
    if consistency and ((fid_loss/batch_size)<1.2 or args.debug):
        ...
        v_resid = rv-v_act-v_doppler_sum
        v_reg_loss = 5e-2*torch.sum((v_resid-v_resid.mean())**2)

        print("v_reg_loss:",v_reg_loss.item()/batch_size)

        K = model.activity_estimator.planet_params[:,0]
        Period = model.activity_estimator.planet_params[:,1]
        ...
        print(f"Initial RMS:{rv.std().item():.3f} m/s")
        print(f"Residual RMS:{(v_resid).std().item():.3f} m/s")

        s_corr = pearson_corrcoef_batch(rv.T,s.T)
        print("s_i correlation with v_encode:",s_corr.detach())

        print_planet_solutions(planet_params,
                               model.activity_estimator.planet_params,
                               corr=doppler_corr)
        ...
        v_act = torch.zeros_like(rv)
        v_doppler = torch.zeros_like(rv)
    if args.debug:
        diags = {"template":template_data,
                 "spec_rest":model.decoder.spec_rest,
                 'planet':planet_param}
        diags['rv'] = [jd,rv,v_planet,v_act,v_doppler]

        if stellar_activity:
            diags["input_data"]=[spec_zero_rv,w,spec_input_aug]
...
def load_model(mainfile, models, instruments, new_act = False):
    ...
    for i, model in enumerate(models):
        ...
        for k, v in model_dict.items():
            if not "activity_estimator" in k:
                filtered[k] = v
                continue
            if new_act:continue
            if  k in init_dict:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-dir","--dir", help="data file directory", default="/scratch/gpfs/yanliang/neid-dynamic/")
    ...
    planet_params = np.loadtxt(f"initial_guess/{basename}_{args.planet_seed}.txt")
    ...
    # data loaders
    trainloaders = [ inst.get_data_loader(f"{args.dir}/processed", select=args.data, which="train",
                     batch_size=args.batch_size, shuffle=args.shuffle) for inst in instruments ]
